...
if(is_array($setting['payment'])) {
	...
	if(!empty($wechat)) {
		...
		if (intval($wechat['switch']) == PAYMENT_WECHAT_TYPE_SERVICE) {
			if (empty($wechat['service'])) {
				$wechat['signkey'] = $setting['payment']['wechat_facilitator']['signkey'];
			} else {
				$pay_service = $wechat['service'];
			}
		} else {
			$wechat['signkey'] = ($wechat['version'] == 1) ? $wechat['key'] : (!empty($wechat['apikey']) ? $wechat['apikey'] : $wechat['signkey']);
			if ($setting['payment']['wechat_facilitator']['switch']) {
				$pay_service = $setting['payment']['wechat_facilitator']['service'];
			}
		}
		if ($pay_service) {
			$facilitator_setting = uni_setting($pay_service, array('payment'));
