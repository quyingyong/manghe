...
if ('front_download' == $do) {
	...
	if (WXAPP_TYPE_SIGN == $_W['account']->typeSign) {
		...
		$version_info['plugins'] = $module_config['support_plugins'];
		$version_info['support_plugins'] = !empty($module_config['support_plugins']) ? STATUS_ON : STATUS_OFF;
	}
	template('wxapp/version-front-download');
}
...
if ('publish' == $do) {
	...
	$params = array(
		...
		'invalid_plugins' => !empty($_GPC['support_live']) ? explode(',', $_GPC['support_live']) : array(),
		'appid' => $appid,
