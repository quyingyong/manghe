...
if ('trim' == $do) {
	$cachekey = cache_system_key('system_db_diff');
	$diff = cache_load($cachekey);

	if ($_W['ispost']) {
		$type = $_GPC['type'];
		$data = $_GPC['data'];
		$table = $_GPC['table'];
		$table_name = str_replace('ims_', '', $table);

		if (!empty($diff)) {
			if ('field' == $type && in_array($data, $diff[$table_name]['fields']) && pdo_fieldexists($table, $data)) {
				$sql = "ALTER TABLE `$table` DROP `$data`";
				if (false !== pdo_query($sql)) {
					cache_delete($cachekey);
					iajax(0, '操作成功', referer());
				}
			} elseif ('index' == $type && in_array($data, $diff[$table_name]['indexes']) && pdo_indexexists($table, $data)) {
