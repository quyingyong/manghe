			...
			<div class="col-sm-8">
				<form action="" method="get" class="form-horizontal form">
							...
							<?php
								...
								<?php if (1 == $development) {
									...
									<option <?php if ($_W['uniacid'] == $account['uniacid']) {
												?>selected<?php
											} ?> value="<?php echo '../api.php?id=' . $account['acid']; ?>&timestamp=<?php echo $timestamp; ?>&nonce=<?php echo $nonce; ?>&signature=<?php echo $signString; ?>&emulator=1"><?php echo $account['name']; ?></option>
								<?php
										} else {
											?>
									<option <?php if ($_W['uniacid'] == $account['uniacid']) {
					...
					<div class="form-group content_type" id="image">
						...
						<div class="col-sm-10 col-xs-12">
							<input type="text" id="picurl" value="logo.png" class="form-control" />
						</div>
					</div>
			...
			<div class="col-sm-4" id="demoSendBox">
				<div class="chatPanel form" style="width:300px;">
					...
					<div class="chatItem you">
							...
							<div class="media mediaFullText">
								<div class="mediaPanel">
									<a href="javascript:;" id="svurl" target="_blank">
										<div class="mediaHead">
											<span class="title" id="svtitle">标题</span>
											<span class="time"><?php echo date('m月d日'); ?></span>
											<div class="clr"></div>
										</div>
						...
						<div id="miniappbox" style="display:none;">
							<div class="media mediaFullText">
								<div class="mediaPanel">
									<a href="javascript:;" >
										<div class="mediaHead">
											<span class="title" id="miniappbox_title">标题</span>
											<div class="clr"></div>
										</div>
										<div class="mediaImg"><img id="miniappbox_pic" src=""></div>
									</a>
									<div id="svinfolist"></div>
									<div class="mediaFooter">
										<div class="mediaFooterbox clearfix">
											<span style="line-height:50px;" class="left">
											<i class="wi wi-wxapp color-default"></i>小程序
											</span>
										</div>
										<div class="clr"></div>
									</div>
	...
	if (1 == $development) {
		...
		function buildRequest(type) {
			...
			if (type == 'text') {
				...
				$('#svpostinfo').html('<img src="<?php  if(!empty($_W['setting']['copyright']['blogo'])) { ?><?php  echo to_global_media($_W['setting']['copyright']['blogo'])?><?php  } else { ?>./resource/images/logo/logo.png<?php  } ?>" style="max-height: 100px;max-width: 200px;">');
					...
					if(xmlobject){
						...
						if(xmlobj.length){
							...
							if(MsgType=='text'){
								...
								var Title = xml.getElementsByTagName("Title")[0].firstChild.nodeValue;
								var PicUrl = xml.getElementsByTagName("PicUrl")[0].firstChild.nodeValue;
								$('#miniappbox_title').html(Title);
								// $('#svinfo').html(Description);
								$('#miniappbox_pic').attr('src', PicUrl);
								$('#miniappbox').show()
							}
						}
					}
