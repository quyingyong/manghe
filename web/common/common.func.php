...
function buildframes($framename = '') {
	...
	if (defined('FRAME') && FRAME == 'account') {
		...
		$modulename = safe_gpc_string($_GPC['module_name']) ? safe_gpc_string($_GPC['module_name']) : safe_gpc_string($_GPC['m']);
	$eid = intval($_GPC['eid']);
	...
	if ((!empty($modulename) || !empty($eid)) && !in_array($modulename, module_system())) {
		...
		if (!empty($frames['wxapp']['section']['publish']['menu']['publish_front_download'])) {
			$frames['wxapp']['section']['publish']['menu']['publish_front_download']['need_upload'] = empty($need_upload) ? 0 : 1;
		}
			...
			foreach ($frames['wxapp']['section'] as $wxapp_section_id => $wxapp_section) {
				...
				if (!empty($wxapp_section['menu']) && 'platform_module' != $wxapp_section_id) {
					foreach ($wxapp_section['menu'] as $wxapp_menu_id => $wxapp_menu) {
						if (in_array($wxapp_section_id, array('wxapp_profile', 'wxapp_entrance', 'statistics', 'mc', 'publish'))) {
	...
	foreach ($frames as $menuid => $menu) {
		if ((!empty($menu['founder']) || in_array($menuid, array('module_manage', 'site', 'advertisement', 'appmarket'))) && !$_W['isadmin'] ||
			...
			if (is_array($_W['setting']['store']['blacklist']) && in_array($_W['username'], $_W['setting']['store']['blacklist']) && !empty($_W['setting']['store']['permission_status']) && $_W['setting']['store']['permission_status']['blacklist'] && 'store' == $menuid ||
				...
				$_W['setting']['store']['status'] == 1 && 'store' == $menuid && !$_W['isadmin']) {
				continue;
			}
...
//					'<a href="{$url}" target="_blank">请尽快完善您在微信魔方云服务平台的站点注册信息。</a>'+
				'</div>'+
			'</div>'+
		'</div>';
	$('body').prepend(html);
