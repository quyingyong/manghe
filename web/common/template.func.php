...
function template_compile($from, $to, $inmodule = false) {
	...
	if (IMS_FAMILY == 'x' && !preg_match('/(footer|header|account\/welcome|login|register|home\/welcome|cloud\/upgrade|cloud\/sms)+/', $from)) {
		$content = str_replace('微信魔方', '系统', $content);
	}
	if (defined('IN_MODULE') &&
		...
		$extra_code = "<script>var we7CommonForModule = document.createElement(\"script\");we7CommonForModule.src = './resource/js/lib/w7windowside.js?v=" . IMS_RELEASE_DATE . "';document.body.appendChild(we7CommonForModule)
</script>";
		$content .= $extra_code;
	}
