...
if (empty($_W['isfounder']) && !empty($_W['user'])) {
	if ($_W['user']['is_expired'] == STATUS_ON) {
		isetcookie('__session', '', -10000);
		$_W['isajax'] ? iajax(-1, '您的账号已到期，请联系管理员解决！', url('user/login')) : itoast('您的账号已到期，请联系管理员解决！', url('user/login'), 'info');
	}
	if ($_W['user']['status'] == USER_STATUS_CHECK || $_W['user']['status'] == USER_STATUS_BAN) {
		isetcookie('__session', '', -10000);
		$_W['isajax'] ? iajax(-1, '您的账号正在审核或是已经被系统禁止，请联系网站管理员解决！', url('user/login')) : itoast('您的账号正在审核或是已经被系统禁止，请联系网站管理员解决！', url('user/login'), 'info');
	}
}
