...
class StoreModuleSite extends WeModuleSite {
	...
	public function getTypeName($type) {
		$sign = array(
			...
			STORE_TYPE_TOUTIAOAPP_RENEW => '字节跳动小程序续费',
			...
			STORE_TYPE_TOUTIAOAPP_MODULE => '字节跳动小程序应用',
		);
	...
	public function payResult($params) {
		...
		if ('success' == $params['result'] && 'notify' == $params['from']) {
			...
			if (!empty($order)) {
				...
				if (empty($_W['uid'])) {
					$_W['uid'] = $order['buyerid'];
					$user = pdo_get('users', array('uid' => $_W['uid']));
					$_W['user']['endtime'] = $user['endtime'];
				}
				...
				if (STORE_TYPE_USER_PACKAGE == $goods['type']) {
					$user = user_single($order['buyerid']);
	...
	public function doWebCashSetting() {
		...
		if (checksubmit('submit')) {
			...
			setting_save($settings, 'store');
			itoast('设置成功！', $this->createWebUrl('cashsetting', array('module_name' => 'store', 'direct' => 1)), 'success');
		}
	...
	public function doWebGoodsSeller() {
		...
		if ('display' == $operate) {
			...
			if (!empty($goods_list)) {
				foreach ($goods_list as &$good) {
					$good['group_timelimit'] = '';
					if (!empty($good['user_group'])) {
						$user_group_info = user_group_detail_info($good['user_group']);
						$good['group_timelimit'] = 0 == $user_group_info['timelimit'] ? '永久有效' : $user_group_info['timelimit'] . '天';
					}
		...
		if ('changestatus' == $operate) {
			$goods = pdo_get('site_store_goods', array('id' => $id));
			if (0 == $goods['status']) {
				$module_name = array();
				if (STORE_TYPE_MODULE == $goods['type']) {
					$module_name = array($goods['module']);
				}
				if (in_array($goods['type'], array(STORE_TYPE_ACCOUNT, STORE_TYPE_ACCOUNT_RENEW, STORE_TYPE_PACKAGE, STORE_TYPE_ACCOUNT_PACKAGE))) {
					$group_info = iunserializer($goods['user_group_price']);
					if (!empty($group_info)) {
						$group_id = array_keys($group_info);
						if (!empty($group_id)) {
							$user_group = pdo_getall('users_group', array('id IN ' => $group_id), array('package'));
							foreach ($user_group as &$package) {
								$package = iunserializer($package['package']);
								if (empty($package)) {
									$package = array();
								}
							}
							$user_group_id = array_reduce((array)$user_group, 'array_merge', array());
							$module_name = module_name_list($user_group_id);
						}
					}
				}
				if (STORE_TYPE_USER_PACKAGE == $goods['type'] && !empty($goods['user_group'])) {
					$user_group = pdo_get('users_group', array('id' => $goods['user_group']), array('package'));
					$module_name = module_name_list(iunserializer($user_group['package']));
				}
				...
				if (!empty($module_expired_list)) {
					...
					if ($expired_modules) {
						itoast('权限组中有服务到期的应用！', referer(), 'error');
					}
				}
			}
	...
	public function doWebGoodsPost() {
			...
			if (checksubmit('submit')) {
				...
				if (!empty($_GPC['user_group_price']) && !empty($_GPC['user_group_id']) && count($_GPC['user_group_price']) == count($_GPC['user_group_id'])) {
					foreach ($_GPC['user_group_price'] as $k => $value) {
						if (empty($value) || empty(safe_gpc_int($_GPC['user_group_id'][$k]))) {
							continue;
						}
						$value = trim($value);
						if (!is_numeric($value)) {
				...
				$good_module = pdo_get('site_store_goods', array('id' => $id));
				$module_name = array();
				if (STORE_TYPE_MODULE == $type) {
					$module_name = array($good_module['module']);
				}
				if (in_array($type, array(STORE_TYPE_ACCOUNT, STORE_TYPE_ACCOUNT_RENEW, STORE_TYPE_PACKAGE, STORE_TYPE_ACCOUNT_PACKAGE))) {
					$group_info = iunserializer($data['user_group_price']);
					if (!empty($group_info)) {
						$good_group_id = array_keys($group_info);
						$good_user_group = array_keys(iunserializer($good_module['user_group_price']));
						$group_id = array_diff($good_group_id, $good_user_group);
						if (!empty($group_id)) {
							$user_group = pdo_getall('users_group', array('id IN ' => $group_id), array('package'));
							foreach ($user_group as &$package) {
								$package = iunserializer($package['package']);
				...
				if (STORE_TYPE_USER_PACKAGE == $type && !empty($data['user_group'])) {
					$user_group = pdo_get('users_group', array('id' => $data['user_group']), array('package'));
					$module_name = module_name_list(iunserializer($user_group['package']));
				}
				$module_expired_list = module_expired_list();
				if (is_error($module_expired_list)) {
		...
		if ('add' == $operate) {
			...
			if (!empty($module_expired_list)) {
				$module_name = explode(',', $data['module']);
				$expired_modules = array_intersect($module_expired_list, $module_name);
				if ($expired_modules) {
					iajax(-1, '应用服务已到期！');
				}
	...
	public function doWebGoodsBuyer() {
		...
		if ('display' == $operate) {
			...
			$goods_table = table('site_store_goods');
			$goods_table->searchWithTypeAndTitle($type, safe_gpc_string($_GPC['keyword']));
		...
		if ('goods_info' == $operate) {
			...
			$goods = table('site_store_goods')->getById($goods);
						if (!$_W['isadmin'] && !$_W['founder'] && !empty($_W['user']['groupid'])) {
				$goods['user_group_price'] = iunserializer($goods['user_group_price']);
			...
			if (STORE_ORDER_PLACE != $order['type']) {
				...
				if (0 == $order['amount']) {
					...
					if (STORE_TYPE_USER_PACKAGE == $goods['type']) {
						...
						$data['groupid'] = $goods['user_group'];
						$data['endtime'] = strtotime('+' . $order['duration'] . $goods['unit'], max($_W['user']['endtime'], time()));
						cache_delete(cache_system_key('system_frame', array('uniacid' => $_W['uniacid'])));
						if (!pdo_update('users', $data, array('uid' => $user['uid']))) {
							iajax(1, '修改权限失败', '');
						}
						user_related_update($user['uid'], $data);
					}
	...
	public function doWebStoreApi() {
		...
		if ('save_wish_goods' == $op) {
			...
			iajax(0, '添加成功', url('site/entry/wishgoodsEdit', array('module_name' => 'store', 'direct' => 1, 'op' => 'wishgoods', 'status' => $common_data['status'])));
		}
	}
	...
	public function doWebCash() {
		...
		if (user_is_vice_founder()) {
			...
			if ('apply_cash' == $operate) {
				...
				itoast('申请成功', $this->createWebUrl('cash', array('direct' => 1, 'module_name' => 'store', 'operate' => 'cash_logs')), 'success');
			}
