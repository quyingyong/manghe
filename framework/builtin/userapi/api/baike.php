...
if (200 == $resp['code'] && $resp['content']) {
	...
	if (!empty($content[0])) {
		...
		if (preg_match_all('|<li>.*<a href="(?<link>.+)">.*<span>(?<title>.+) - 百度百科</span>.*<p>(?<description>.+)</p>.*</a>.*</li>|isU', $content, $matchs)) {
			$ds = array();
			...
			$news = array();
			if (!preg_match('/^http(s)?:\\/\\/.+/',$ds[0]['link'])) {
				$ds[0]['link'] = 'https://baike.baidu.com' . $ds[0]['link'];
			}
