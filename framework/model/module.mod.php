...
function module_upgrade_info($modulelist = array()) {
	...
	if (!empty($cloud_m_query_module) && !is_error($cloud_m_query_module)) {
		//return array();
	...
	foreach ($modulelist as $modulename => $module) {
		...
		pdo_update('modules', array('label' => $module_upgrade_data['label'], 'status' => 1), array('mid' => $module['mid']));
...
function module_expired_list() {
	...
	if (!empty($module_list)) {
		...
		foreach ($module_list as $module_name) {
			...
			if (empty($module) || empty($module['site_branch']['status'])) {
				continue;
			}
			if (!empty($module['service_expiretime']) && $module['service_expiretime'] < $time && 2 != $module['system_shutdown'] && 3 != $module['system_shutdown']) {
...
function module_name_list($group_id = array()) {
	...
	$group_module = pdo_getall('uni_group', array('id IN ' => $group_id), array('modules'));	if (empty($group_module)) {
		return $module_name_list;
	}
