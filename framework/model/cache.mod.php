...
function cache_build_account_modules($uniacid = 0, $uid = 0) {
	...
	if (!empty($uni_modules_delete)) {
               $modules_info = pdo_getall('wxapp_versions', array('uniacid' => $uniacid, 'type in' => array(WXAPP_CREATE_MODULE, WXAPP_CREATE_MUTI_MODULE)), array('modules'));
		if (!empty($modules_info)) {
			$modules = '';
			foreach ($modules_info as $module_info) {
				$modules_name = array_keys(iunserializer($module_info['modules']));
				$modules .= implode(',', $modules_name) . ',';
			}
		}
		foreach ($uni_modules_delete as $mdoule_delete_name) {
			if (!empty($modules) && in_array($mdoule_delete_name, array_unique(explode(',', rtrim($modules, ','))))) {
				continue;
			}
...
function cache_build_uninstalled_module() {
	...
	foreach ($module_path_list as $path) {
		...
		if (!empty($modulelist[$modulename])) {
			...
			if (!empty($module_cloud_upgrade)) {
				...
				if (empty($has_new_support)) { //by qwj418
		...
		if (!empty($modulelist[$modulename])) {
			...
			if (!empty($new_support)) {
				...
				//table('modules_cloud')->deleteByName($modulename); //by qwj418
				continue;
			}
		}
	...
	/*加入微官网模版安装 by RubySn0w*/
	$installed_template = pdo_getall('modules', array('application_type' => APPLICATION_TYPE_TEMPLATES), 'name', 'name');
	...
	if (is_dir($path)) {
		...
		if (!empty($dir_tree)) {
			foreach ($dir_tree as $modulepath) {
				$template_name = pathinfo($modulepath, PATHINFO_BASENAME);//str_replace(IA_ROOT . '/app/themes/', '', $modulepath);
				$module_recycle_info = table('modules_recycle')->searchWithNameType($template_name, MODULE_RECYCLE_UNINSTALL_IGNORE)->get();
				if (!is_dir($modulepath) || !file_exists($modulepath . '/manifest.xml')) {
			        continue;
		        }
				$template_manifest = ext_template_manifest($template_name, false);
				if (!empty($_GPC['title']) && !strexists($template_manifest['title'], trim($_GPC['title']))) {
					continue;
				}
				if (!empty($template_manifest) && !in_array($template_manifest['name'], array_keys($installed_template))) {
                $template_manifest['application_type'] = APPLICATION_TYPE_TEMPLATES ;
				if (file_exists(IA_ROOT . '/app/themes/' . $template_manifest['name'] . '/preview.jpg')) {
			      $template_manifest['logo'] = $_W['siteroot'].'app/themes/'.$template_manifest['name'].'/preview.jpg';
		        } else {
			      $template_manifest['logo'] = $_W['siteroot'].'web/resource/images/nopic-203.png';
		        }
            $template_manifest = array_merge($template_manifest,array(
			'has_new_version' => 0,
			'has_new_branch' => 0,
			'install_status' => MODULE_LOCAL_UNINSTALL,
			'version' => $template_manifest['version'],
			'title_initial' => get_first_pinyin($template_manifest['name']),
			'account_support' => MODULE_SUPPORT_ACCOUNT,
			'module_status' => 1,
		...
		if (!empty($module_recycle_info)) {
			foreach ($module_support_type as $support => $value) {
				if ($module_recycle_info[$support] == 1) {
					$template_manifest[$support] = $value['not_support'];
				}
			}
		}
		$module_cloud_upgrade = table('modules_cloud')->getByName($template_name);
		if (empty($module_cloud_upgrade)) {
			table('modules_cloud')->fill($template_manifest)->save();
		} else {
			table('modules_cloud')->fill($template_manifest)->where('name', $template_name)->save();
		}
				}
			}
		}
...
function cache_build_proxy_wechatpay_account() {
	...
	if (!empty($uniaccounts)) {
		foreach ($uniaccounts as $uniaccount) {
			...
			if (!empty($account['key']) && !empty($account['secret']) && in_array($account['level'], array (ACCOUNT_SERVICE_VERIFY)) &&
