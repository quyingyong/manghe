...
function miniapp_version_all($uniacid) {
	...
	if (!empty($miniapp_versions)) {
		...
		foreach ($miniapp_versions as $key => &$version) {
			...
			if (!isset($user_modules['modules']['permission']) && !empty($user_modules) && !empty(array_diff(array_keys($version['modules']), $user_modules))) {
...
function miniapp_code_generate($version_id, $user_version, $plugins = array()) {
	...
	if (ACCOUNT_TYPE_APP_AUTH == $_W['account']['type']) {
		...
		if (empty($_W['setting']['platform']['bindappid'])) {
			return error(1, '未设置开放平台绑定的开发小程序，无法给该授权小程序上传，请先<a href="./index.php?c=system&a=platform" class="color-default">绑定开发小程序</a>');
		}
