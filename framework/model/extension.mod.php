...
function ext_module_manifest_parse($xml) {
	...
	$xml = preg_replace('/[\\x00-\\x08\\x0b-\\x0c\\x0e-\\x1f\\x7f]/', '', $xml); 		ob_start();
	ob_clean();
	$dom = new DOMDocument();
	$dom->loadXML($xml);
...
function ext_template_manifest_parse($xml) {
	...
	$manifest['version'] = strval($xml->version)? : '1.0'; //by rubysn0w
...
function ext_module_run_script($manifest, $scripttype) {
	...
	//RubysSn0w 修复如果数据中含.php安装数据错误问题
		if (strexists($manifest[$scripttype], '.php') && file_exists($module_path . $manifest[$scripttype])) { 
