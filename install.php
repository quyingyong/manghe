	...
	if($ispost) {
		...
		if(empty($error)) {
			...
			if($_POST['type'] == 'remote') {
				...
				if($ins == 'error') {
					die('<script type="text/javascript">alert("版本错误，请确认是否为微信魔方最新版安装文件！");history.back();</script>');
				}
			...
			if(file_exists(IA_ROOT . '/index.php') && is_dir(IA_ROOT . '/web') && file_exists($verfile) && file_exists($dbfile)) {
				...
				$tmp1 =  mt_rand(100000,999999);//local_salt(6);
				$tmp2 = $_URLTYPE . $_SERVER['HTTP_HOST'];
                $site_tmp = array (  'key' => $tmp1,  'token' => md5($tmp1),  'url' => $tmp2,  'version' => '2.0.0',  'family' => 'x',  'type' => 'register',);
...
EOF;
	           //end			
				...
				die('<script type="text/javascript">alert("你正在使用本地安装, 但未下载完整安装包, 请从微信魔方官网下载完整安装包后重试.");history.back();</script>');
			}
...
<html lang="zh-cn">
	<head>
		...
		<title>安装系统 - 微信魔方 - 公众平台自助开源引擎</title>
		...
		<div class="container">
			<div class="header" style="margin:15px auto;">
				<ul class="nav nav-pills pull-right" role="tablist">
					<li role="presentation" class="active"><a href="javascript:;">安装微信魔方</a></li>
					<li role="presentation"><a href="https://www.tongbayun.com">通霸云互站</a></li>
					<li role="presentation"><a href="hhttps://www.tongbayun.com/question?type=new_question">问题反馈</a></li>
				</ul>
				<img src="?res=" />
			</div>
			...
			<div class="footer" style="margin:15px auto;">
				<div class="text-center">
					<a href="https://www.tongbayun.com">关于微信魔方</a> &nbsp; &nbsp; <a href="https://www.tongbayun.com">通霸云互站</a> &nbsp; &nbsp; <a href="https://www.tongbayun.com/question?type=new_question">问题反馈</a>
				</div>
				<div class="text-center">
					Powered by <a href="https://www.tongbayun.com"><b>微信魔方</b></a>  &copy; 2018 <a href="https://www.tongbayun.com">ym.qingshan.site</a>
				</div>
			</div>
		</div>
		...
		<form class="form-inline" role="form" method="post">
			<ul class="pager">
				<li class="pull-left" style="display:block;padding:5px 10px 5px 0;">
					<div class="checkbox">
						<label>
							<input type="checkbox"> 点击“继续安装”
						</label>
					</div>
				</li>
				<li class="previous"><a href="javascript:;" onclick="if(jQuery(':checkbox:checked').length == 1){jQuery('form')[0].submit();}else{alert('您必须点√才能安装！')};">继续安装 <span class="glyphicon glyphicon-chevron-right"></span></a></li>
			</ul>
		</form>
EOF;
	tpl_frame();
...
function tpl_install_env($ret = array()) {
	...
	echo <<<EOF
		...
		<div class="alert alert-info">系统要求微信魔方整个安装目录必须可写, 才能使用微信魔方所有功能。</div>
		<div class="panel panel-default">
		...
		<div class="panel panel-default">
			...
			<div class="panel-body">
				...
				<div class="form-group">
					...
					<div class="col-sm-4">
						<input class="form-control" type="text" name="db[name]" value="wxmf">
					</div>
				</div>
			</div>
		</div>
...
function tpl_install_finish() {
	...
	<div class="alert alert-success">
		恭喜您!已成功安装“微信魔方 - 公众平台自助开源引擎”系统，您现在可以: <a target="_blank" class="btn btn-success" href="./web/index.php">访问网站首页</a>
	</div>
EOF;
	tpl_frame();
}
